	...
	try {
		await g_coUsers.insertOne({
			...
			password: a_oRequest.cookies.m_sPassword ?
				await g_coBcrypt.hash(a_oRequest.cookies.m_sPassword, process.env.m_saltRounds) : "",
		...
		console.error("Cannot create user due to: ", a_oError)
...
g_coRouter.get("/", g_coAuth, g_cookieParser(), async function(a_oRequest, a_oResponse) {
	let l_vResults
	try {
...
g_coRouter.put("/", g_coAuth, g_cookieParser(), async function(a_oRequest, a_oResponse) {
	try {
		await g_coUsers.updateMany(g_coFilter(a_oRequest.cookies), { $set: {
				username: a_oRequest.get("Username"),
				password: await g_coBcrypt.hash(a_oRequest.get("Password"), process.env.m_saltRounds),
...
g_coRouter.delete("/", g_coAuth, g_cookieParser(), async function(a_oRequest, a_oResponse) {
	try {
